...
public class SearcherFilter {
	...
	public void SearchFilter(String query,Filter filter)throws IOException
	{
	try {
		...
		try {
			...
			if(filter!=null)
			{
			tds=searcher.search(q, filter, 5);
			}
			else
			{
			tds=searcher.search(q,5);
			}
			
			//TopDocs tds=searcher.search(q,filter,5);
			int i=0;
